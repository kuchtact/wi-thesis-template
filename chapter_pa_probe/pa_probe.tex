...
Now we don't know how our temperatures can be represented using a trimaxwellian so we are working to discover what the temperatures and their directions are. Thus let's use the unprimed coordinates $\theta$ and $\phi$ to represent the angles relative to whatever set of axis are convenient (in our case the machine cylindrical coordinates).

We can construct a tensor from this inverse temperature equation by populating each element in the matrix, $T$, via
\begin{align*}
...
Now we can diagonalize this tensor into $T = P D P^{-1}$ where this just acts to rotate this arbitrarily directed trimaxwellian into a coordinate system aligned with $\hat{t}_1$ and $\hat{t}_2$. The eigenvectors give the directions of each temperature and the eigenvalues $\lambda_i$, which make up the diagonal of $D$, are just $T_{ij}'$ (the elements of $T$ in the coordinate system aligned with the temperatures).

Thus we can solve for $y_{l,m}'$ by solving
\begin{align}
    \begin{bmatrix}
        \lambda_1 \\
        \lambda_2 \\
...
\end{align*}
These equations can be solved for the temperatures which along with the eigenvectors gives us the trimaxwellian temperatures and directions.
...
To measure the plasma parameters we first need to do some cleanup by combining multiple shots and then we use data from both the shell and core to get the final plasma parameters of interest.
