...
\begin{equation}
    f_e(\vb{v}) = \left(\frac{m_e}{2\pi k}\right)^{3/2} \frac{n}{\sqrt{T_{e\perp}^2 T_{e\parallel}}} \exp\left[-\frac{e m_e}{2} \left(\frac{(\vb{v} - \vb{v_D})_\parallel^2}{T_{e\parallel}} + \frac{(\vb{v} - \vb{v_D})_\perp^2}{T_{e\perp}}\right)\right].
...
\begin{align}
    A(n_e, T_e; V_\text{plasma}) &= n_e e \sqrt{\frac{e T_e}{m_i}} \left[\frac{1}{2} \sqrt{\frac{2 m_i}{\pi m_e}} \exp\left(\frac{- V_\text{plasma}}{T_e}\right)\right] \\
    B(T_e) &= T_e \\
...
\subparagraph{Temperature by direction}
...
Monte-carlo % TODO: Finish this once I have settled on a method.
