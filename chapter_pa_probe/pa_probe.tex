...
\begin{align}
    \sigma_{ne} &= \partial_A n_e(A, B) \sigma_A + \partial_B n_e(A, B) \sigma_B \\
    \sigma_{Te} &= \sigma_B \\
    \sigma_{ni} &= \partial_C n_e(C, B) \sigma_C + \partial_B n_e(C, B) \sigma_B.
\end{align}
...
If we ignore the drift velocity in the distribution function, equation \ref{eq:drifting_bimaxwellian}, we find that the term in the exponential coefficient takes the form
\begin{equation}
...
\begin{figure}
    ...
    \caption[Lightsaber Probe Measurements]{This figure shows three different shots (columns) and their respective Bdot measurements in various directions (rows). Most figures are like that of shot 69728 where there is a single x-point for many of our configurations but sometimes during a shot, plasmoids can form like those in shots 69664 and 69687. These plasmoids can form either to the left or right of the main reconnecting x-point and can change the outflow dynamics of the reconnecting current layer.}
